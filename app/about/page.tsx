...
const EMPTY_FORM: FormState = { name: "", email: "", message: "", honeypot: "" };
...
export default function AboutPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.name.trim() || !form.email.trim() || !form.message.trim() || !EMAIL_REGEX.test(form.email)) {
      triggerShake();
      return;
    }
            ...
            {status !== "success" ? (
              <>
                {status === "error" && (
                  <div
                    className="mono"
                    style={{ color: "var(--magenta)", fontSize: 12, marginBottom: 14, letterSpacing: "0.04em" }}
                  >
                    {errorMsg}
                  </div>
                )}
